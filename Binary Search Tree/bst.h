...
public:
    ...
    std::string repeat_string(std::string s,int n){
        std::string s1 = s;
    ...
    void print_intermediate(BTreeNode<T> *node){
        if(node!= NULL){
            count++;
            std::cout << repeat_string("--",count) << node->get_val() << std::endl;
            if((node->get_left())!=NULL){
                count++;
                print_intermediate(node->get_left());
            }
            if((node->get_right())!=NULL){
                count++;
                std::cout << "--";
                print_intermediate(node->get_right());
            }
            count -= count;
        }
    }
